...
Excellent. We have now reached **the single most important chapter of Week 2**.

If someone asks an SRAM circuit designer:

> **"How do you know your SRAM cell is stable?"**

The answer almost always starts with:

> **Static Noise Margin (SNM).**

This is one of the first analyses performed on a newly designed SRAM bitcell before moving on to timing, yield, or full memory-array integration.

---

# Week 2 – Chapter 9

# Static Noise Margin (SNM) & Butterfly Curve Analysis

> **Objective:** Understand the theory behind Static Noise Margin (SNM), learn how the butterfly curve is generated, recreate the simulations using xschem/ngspice with SKY130 models, and interpret the results as an SRAM circuit designer.
...
* Explain what Static Noise Margin (SNM) is.
* Understand why SNM is the primary stability metric.
* Derive the butterfly curve from inverter characteristics.
* Differentiate between Hold SNM, Read SNM, and Write SNM.
* Generate butterfly curves using DC sweep simulations.
* Estimate SNM using the largest embedded square method.
* Understand how transistor sizing, VDD, and process variation affect SNM.
* Document results in a GitHub-ready format.

---

# 1. Why Do We Need SNM?

A successful simulation at nominal conditions does **not** guarantee that the SRAM cell is robust.

Real silicon experiences:
...
SNM answers the question:

> **How much DC noise can the SRAM cell tolerate before it changes state?**

---

# 2. Definition of Static Noise Margin

Static Noise Margin is the **maximum DC noise voltage** that can be superimposed on the storage nodes without causing the cell to lose its stored data.
...
NO → Noise exceeds SNM
```

SNM is therefore a direct measure of stability.

---

# 3. Why "Static"?

The term **static** indicates:

* DC operating condition.
* No clock transitions.
* No dynamic switching.

Dynamic noise analysis is a separate topic.

---

# 4. Understanding Cross-Coupled Inverters

A 6T SRAM cell is essentially:
...
Each inverter attempts to reinforce the other.

This regenerative loop creates **two stable operating points** and **one unstable operating point**.

---

# 5. Voltage Transfer Characteristic (VTC)

Before generating a butterfly curve, characterize a **single inverter**.
...
This is called the **Voltage Transfer Characteristic (VTC)**.

---

# 6. Generating the Butterfly Curve

![Image](https://images.openai.com/static-rsc-4/0yb4Y8FCvF-eCGc-1o0wbtVlnYfQaiyqOQYoJtJX0HQLvOOJbMpViJkVY5ziKwZ7hJ3HIx8m99MMmlMHEwwiOn-JIWU8WAG2imIQZH-Bm9aDSgeit5osf2JvHs7OwSEL_jbSLVPsVEUtULMP7ijnAS5zm_8O7R77lDJgs9pM9vsR2BwVPsbVuFxD-isXJDO0?purpose=fullsize)

![Image](https://images.openai.com/static-rsc-4/rPhoYPhEzjUY7xwB7uXXOCiIxrIHsQ_yxKEtxuQcdVjUbtIaEwd_UPgDZHF1z7BM7FB7CXdR5N4xj6uFGqs-PDY-l0QFxRvX3bNRLcNxHsHuM2EqIgHeldNY2Vdkgy_Kaups3GBqe95CBXfBVO7-ZQf98WKRrnpn5tezCbgQ67frMgsT4Cp53rg_SwU3Vz8s?purpose=fullsize)

![Image](https://images.openai.com/static-rsc-4/70TfwDYavhBiFEVjgLHPOascQ909InnybG-cA_LxuWQ6t4jrb29Y2RPBn8wl7sxVmylNVDrhmEXJN_q0eU5WdGEKWkRTrp2mf7G5AgjgbbxVDb9F7w0BuA8XWd8IXCIcqWH7jk-p7YiJtCvScTy9KB_tv1aLbgTXpfdiyyTamVqLVssXV8aqWhIQT1WlU3z0?purpose=fullsize)

![Image](https://images.openai.com/static-rsc-4/Vy6kalsKTQM2GtFua8rD6becPdlfSt9JjYCfl7pWZ6z-W6F7jwLLdk-AraYiex_g4gWq8o16-1XFPtD7KrRTVROHMVe7YxAIzEBuHQ-WF4_cjgpfq3mHd4aXUQaWcK7sxdeeSjDI7Gr4eBPt_vLskKytkerXXZNdz1o_dV3YwiNc3lqXeGfgbcIRQNxEVbeE?purpose=fullsize)

The butterfly curve is generated by:

1. Plotting Inverter 1 VTC.
2. Mirroring Inverter 2 VTC.
3. Superimposing both curves.
...
The enclosed region resembles butterfly wings, giving the curve its name.

---

# 7. Largest Embedded Square Method

The most common way to estimate SNM is:

1. Draw the butterfly curve.
2. Fit the **largest possible square** between the two lobes.
3. The side length of this square is the SNM.

This method provides a graphical measure of cell stability.
...
This is usually the **highest** SNM because the cell is isolated.

---

### Read SNM

Measured during an active read.

The access transistors connect the storage nodes to the precharged bitlines, reducing stability.
...
Associated with the cell's resistance to being overwritten.

Lower write SNM generally indicates easier writing.

---

# 9. Relationship Between Stability Metrics
...
These metrics together define SRAM robustness.

---

# 10. Factors Affecting SNM

| Parameter             | Effect                                            |
| --------------------- | ------------------------------------------------- |
| Higher VDD            | Generally increases SNM                           |
| Lower VDD             | Reduces SNM                                       |
| Strong pull-down NMOS | Improves read stability                           |
| Strong pull-up PMOS   | Improves hold stability                           |
| Strong access NMOS    | Can reduce read SNM while improving write ability |
| Device mismatch       | Reduces worst-case SNM                            |
| Higher temperature    | Usually reduces stability                         |
...
> Explain Static Noise Margin (SNM) in a 6T SRAM cell and describe the butterfly curve.

### Prompt 2 – Simulation

> Generate an ngspice DC sweep testbench to obtain the voltage transfer characteristics of a CMOS inverter using SKY130 models.

### Prompt 3 – Characterization

> Explain how to construct an SRAM butterfly curve from two inverter VTCs.

### Prompt 4 – Verification

> Review this SNM simulation setup and identify any missing sweeps or incorrect bias conditions.

### Prompt 5 – Debugging

> Why is my butterfly curve asymmetric?

---

# 12. xschem Exercise

1. Build a CMOS inverter using SKY130 devices.
2. Perform a DC sweep:

   * Sweep Vin from 0 V to VDD.
3. Plot:

   * Vout vs Vin.
4. Mirror the second inverter characteristic.
5. Construct the butterfly curve.
6. Estimate the largest embedded square.

Repeat for:

* Hold mode.
* Read mode (with WL asserted).

Compare the difference qualitatively.
...
To study supply sensitivity, repeat the DC sweep at different VDD values (for example, 1.8 V, 1.5 V, and 1.2 V) and compare how the VTC shifts.
...
| Observation                       | Likely Cause                                    | Suggested Check                        |
| --------------------------------- | ----------------------------------------------- | -------------------------------------- |
| Butterfly not formed              | Incorrect inverter mirroring                    | Verify VTC construction                |
| Strong asymmetry                  | PMOS/NMOS sizing imbalance                      | Review transistor widths               |
| SNM appears unrealistically large | Wrong bias conditions                           | Confirm WL and bitline states          |
| Convergence problems              | DC sweep step too coarse or poor initialization | Refine sweep settings                  |
| Different results between runs    | Inconsistent setup                              | Ensure identical simulation conditions |

---

# 15. Industry Characterization Flow

A typical SRAM characterization sequence:

1. Generate inverter VTC.
2. Generate butterfly curve.
3. Measure Hold SNM.
4. Measure Read SNM.
5. Repeat across:
...
7. Compare statistical distributions.

For Week 2, focus on steps 1–4.
...
* AI prompts.
* VTC plots.
* Butterfly curve screenshots.
* Estimated SNM.
* Comparison of Hold and Read SNM.
* Notes on simulation methodology.
...
* What is the Hold SNM?
* What is the Read SNM?
* How much SNM is lost during a read?
* How does SNM change at lower VDD?
* What is the statistical distribution under mismatch?

Quantitative answers—not qualitative statements—drive design decisions.

---

# 18. Advanced Industry Perspective (Beyond Assignment)

Although your assignment stops at circuit-level understanding, advanced SRAM development also evaluates:

* **Monte Carlo SNM** for random mismatch.
* **Process-corner SNM** (SS, TT, FF, SF, FS).
* **Temperature-dependent SNM**.
* **Near-threshold operation** for ultra-low-power SRAM.
* **Assist techniques** (wordline boost, negative bitline, supply collapse) to recover SNM at low voltages.

These topics become important in production memory IP but are built upon the SNM concepts introduced here.

---

# 19. Chapter Summary

This chapter introduced **Static Noise Margin (SNM)** as the primary quantitative measure of SRAM stability. We studied inverter voltage transfer characteristics, constructed the butterfly curve, learned the largest embedded square method, and differentiated between Hold SNM, Read SNM, and Write-related stability metrics. We also established an AI-assisted workflow for generating DC sweep simulations, documenting butterfly curves, and interpreting the results from an SRAM designer's perspective.

---

# Week 2 Progress Check

At this stage, you have covered the **core bitcell topics**:

* ✅ Chapter 3 – 6T SRAM Architecture
* ✅ Chapter 4 – Hold Operation
* ✅ Chapter 5 – Read Operation
* ✅ Chapter 6 – Read Disturb
* ✅ Chapter 7 – Write Operation
* ✅ Chapter 8 – Write Margin
* ✅ Chapter 9 – Static Noise Margin (SNM)

This represents the heart of **Week 2** and provides the theoretical and simulation foundation required before moving into **Week 3**, where the focus shifts from the bitcell itself to the **peripheral circuits** that enable an SRAM array: precharge, equalization, wordline drivers, write drivers, sense amplifiers, row/column decoders, timing, and complete read/write sequencing.

## Preview of Chapter 10 – Cell Ratio (CR) & Pull-Up Ratio (PR)

Before leaving the bitcell, we will study two critical sizing metrics:

* **Cell Ratio (CR)** – Governs read stability.
* **Pull-Up Ratio (PR)** – Governs write ability.

These ratios connect all the previous chapters by explaining **why transistor sizing choices affect Read Disturb, Write Margin, and SNM**. This chapter bridges theoretical understanding and practical SRAM sizing methodology used in industry before we transition into Week 3 peripheral circuit design.


Excellent. **Chapter 10 is the bridge between Week 2 and Week 3.**

Until now, we have studied **what happens** during hold, read, write, read disturb, write margin, and SNM.

Now we answer the engineering question:

> **Why does changing one transistor width improve one metric while degrading another?**

The answer lies in **transistor sizing**.

This chapter is one of the most important topics for anyone working in SRAM circuit design because almost every optimization starts with these ratios.

---

# Week 2 – Chapter 10

# SRAM Transistor Sizing, Cell Ratio (CR) & Pull-Up Ratio (PR)

> **Objective:** Learn how transistor sizing affects SRAM stability, understand Cell Ratio (CR) and Pull-Up Ratio (PR), study sizing trade-offs, recreate sizing experiments using AI-assisted SPICE simulations, and document the characterization results.
...
* Understand why SRAM transistor sizing is critical.
* Define Cell Ratio (CR).
* Define Pull-Up Ratio (PR).
* Explain how CR affects read stability.
* Explain how PR affects write ability.
* Perform transistor sizing experiments using ngspice.
* Correlate sizing changes with SNM and write margin.
* Understand industry sizing methodology.

---

# 1. Why Transistor Sizing Matters

Unlike digital standard cells, the six transistors in an SRAM bitcell are **not identical**.

Each transistor has a different job:

* Pull-up PMOS → maintain logic '1'
* Pull-down NMOS → maintain logic '0'
* Access NMOS → connect the cell to the bitlines

If all six transistors have the same width, the cell may function, but it is rarely optimal for stability, speed, or manufacturability.

---

# 2. The Six Transistors

![Image](https://images.openai.com/static-rsc-4/L1G6k1y61K1RpkW9IjIJjVEKDAfrZU1g1PBKFTv08A3YrusAxb5poSPZ-Ymj6yVuBVifcXL8oykjMzy-qOj_1qURyB-CoSU4OJ7nsLJBTHeWkp9TA-SsqMwk_UpZK6OhmoEg9tgylKWxTurnFzCIm-9iQGhHSXqUEGK4XgO-MFfIUvnKji8DK_Jngb9rApiI?purpose=fullsize)

![Image](https://images.openai.com/static-rsc-4/hPmb_yTmmA-Nqc-PX2JXS2xAwDpoKEAg_5uS_8MwGQVdjbwhzjrGtUsSSxpp6IK_A8aqfnkZpf8G4YQ515CEf_Vd75gy_voAeJWwl6tMP4FzRZFoy3GOmkpC--GI_2V9Pd-IYRXLQJ5Uhvhduin_0LHOrVsD0W_99Zw_yn99XNmxZNMJJG1voWaI8fo3dKZT?purpose=fullsize)

![Image](https://images.openai.com/static-rsc-4/4ETKDTDqDiMZJ_pOAFy9tWwlE-DbrkpJWKyYo43q-Ci2haJVF7De4oI34bJ1tj9yhCBFoAYdjnNcmDnl7T3ZsO9TRu3-8PNHaS876Y47KtpqGilr80QLOEdv1qkH1GW4eGWV908h0kHhiYEzM2Wcl5NHtlxpes4MnRnSPH78qPBSnexpKOHdDOHxn9ACTV-n?purpose=fullsize)

![Image](https://images.openai.com/static-rsc-4/cJzKDu4dNrk7nu3WmcXpf_bDyvw3BypAP60LpyiAXZqojh0uncvzFw8NGKPUZWi2sHFNTQZohanxdoZIzF5LHzks1XtCvvT4o-G6omnwXVR-Mcp9_Tik9cYM5nFtjGylcZnzPPEiueYghIyqT5FBxKpsM7FHa5zq_RfZ1a9IKc0hvcTw4Q6upj6PK5YShN-W?purpose=fullsize)

![Image](https://images.openai.com/static-rsc-4/rQ1gVJ3N-Bttro0IwlUXsOmFbqvQ2pUgRp7D7J_l3NFsOHRpGR7SYOfH77XN6CMQW5ZRKByC_429ImzEtOfb80HyFbB2blQz0YnfZJn9LTBjI-yvJ5Q0_HNQ5zeEo6AiJm3p5vNW_LQT3YxT6miz9JqfXbaaCW55Xc58s9o9Dww_C_0d5etZXyuWtpBELKjU?purpose=fullsize)

The transistor groups are:
...
The relative widths of these devices determine the overall behavior of the cell.

---

# 3. Cell Ratio (CR)

Cell Ratio is one of the most important SRAM design parameters.
...
Access NMOS Strength
```

A first-order approximation using transistor widths is:

```text
CR ≈ Wpull-down / Waccess
```

where the channel lengths are assumed equal.

---

# 4. Why Cell Ratio Matters
...
Two transistors compete:

* Access NMOS tries to raise the internal node (through charge sharing).
* Pull-down NMOS tries to keep the node near ground.

A larger CR means:

* Stronger pull-down.
* Better resistance to read disturb.
* Higher Read SNM.

---

# 5. Effect of Cell Ratio

| Cell Ratio | Read Stability         |
| ---------- | ---------------------- |
| Too small  | Poor                   |
| Moderate   | Good                   |
| Very large | Stable but larger area |

Increasing CR generally improves read robustness, but it also increases silicon area and parasitic capacitance.

---

# 6. Pull-Up Ratio (PR)

The Pull-Up Ratio compares the access transistor against the pull-up PMOS.
...
A width-based approximation is:
...
# 7. Why Pull-Up Ratio Matters

During a write of logic '0':
...
The access transistor must overcome the pull-up PMOS.

A larger PR generally means:

* Easier writing.
* Improved write margin.

However, excessive access strength may reduce read stability.

---

# 8. Engineering Trade-Off

This is the central optimization problem in SRAM design.

| Increase       | Benefit               | Cost                       |
| -------------- | --------------------- | -------------------------- |
| Pull-down NMOS | Better read stability | Increased area/capacitance |
| Access NMOS    | Better write ability  | More read disturb          |
| Pull-up PMOS   | Better hold stability | Harder writes              |

No single sizing is optimal for every design objective.

---

# 9. Typical Design Targets

The exact values depend on technology, but designers usually seek:

* Adequate Read SNM.
* Adequate Write Margin.
* Acceptable Hold SNM.
* Reasonable access time.
* Minimum silicon area.

Sizing is therefore an iterative optimization process rather than selecting fixed ratios.
...
> Explain Cell Ratio (CR) and Pull-Up Ratio (PR) in a 6T SRAM cell.

### Prompt 2 – Sizing

> Generate an ngspice testbench to compare three different access transistor widths while keeping other devices constant.

### Prompt 3 – Characterization

> Explain how changing pull-down NMOS width affects read stability.

### Prompt 4 – Verification

> Review this SRAM sizing configuration and identify potential read or write weaknesses.

### Prompt 5 – Debugging

> Why does increasing the access transistor width improve write ability but reduce read stability?

---

# 11. xschem Exercise

Starting from the validated bitcell:
...
# 12. Suggested ngspice Sweep

Example parameterized width:

```spice
.param WACC=0.42u
```

Then evaluate several values:
...
# 13. Characterization Table

| Case | Access Width | CR Trend | PR Trend | Read Stability | Write Ability |
| ---- | ------------ | -------- | -------- | -------------- | ------------- |
| A    | Small        | High     | Low      | Strong         | Weak          |
| B    | Medium       | Balanced | Balanced | Good           | Good          |
| C    | Large        | Low      | High     | Reduced        | Strong        |

This table illustrates the qualitative trade-offs. Your simulation results will provide quantitative evidence.

---

# 14. Common Debugging Issues

| Observation                     | Likely Cause                           | Suggested Investigation                           |
| ------------------------------- | -------------------------------------- | ------------------------------------------------- |
| Excellent writes but poor reads | Access transistor oversized            | Review CR                                         |
| Strong hold but failed writes   | Pull-up PMOS oversized                 | Review PR                                         |
| Good reads but slow access      | Pull-down devices too large            | Check delay and capacitance                       |
| Highly asymmetric behavior      | Device mismatch or inconsistent sizing | Verify dimensions                                 |
| Unexpected trend                | Simulation setup inconsistency         | Confirm that only one parameter changes at a time |

---

# 15. Industry Characterization Flow

Professional SRAM sizing typically follows:

1. Establish initial transistor widths.
2. Verify hold operation.
3. Measure Read SNM.
4. Measure Write Margin.
5. Adjust CR and PR.
6. Repeat simulations.
7. Perform PVT and Monte Carlo characterization.
8. Freeze the bitcell sizing.

This iterative loop is repeated until the required yield targets are achieved.
...
```text
Week2/
└── Chapter10_Transistor_Sizing/
    ├── README.md
    ├── prompts.md
    ├── spice/
    │   └── sizing_sweep.sp
    ├── xschem/
    │   └── parameterized_bitcell.sch
    ├── waveforms/
    ├── screenshots/
    ├── sizing_table.md
    ├── observations.md
...
* Comparison tables.
* Waveforms.
* Personal conclusions about CR and PR trade-offs.
...
> "What transistor width should I use?"

An SRAM circuit designer asks:

* What Read SNM is required?
* What Write Margin is required?
* What VDD range must be supported?
* What process corners must pass?
* What area budget is available?

The transistor widths are chosen only after answering these system-level questions.

---

# 18. End of Week 2

At this point, you have completed the complete **bitcell-focused study**:
...
* ✅ Write margin.
* ✅ Static Noise Margin.
* ✅ Transistor sizing with Cell Ratio and Pull-Up Ratio.

These chapters establish the knowledge required to understand why the bitcell behaves the way it does under different operating conditions.

---

# Transition to Week 3

Week 3 shifts the focus from the isolated bitcell to the **supporting peripheral circuits** that make an SRAM array functional. The study progresses in the same sequence used in industry:

1. Precharge and Equalization.
2. Wordline Driver.
3. Bitline Behavior.
4. Sense Amplifier.
5. Write Driver.
6. Row Decoder.
7. Column Decoder.
8. SRAM Timing Sequence.
9. Integration of the complete read/write path.

These peripherals are studied individually before assembling the complete SRAM access path.

---

# Chapter Summary

This chapter explained how **transistor sizing governs SRAM behavior**. We introduced the **Cell Ratio (CR)** as the key metric for read stability and the **Pull-Up Ratio (PR)** as an important indicator of write ability. Rather than treating transistor dimensions as arbitrary values, we connected sizing decisions directly to Read Disturb, Write Margin, and Static Noise Margin, providing the conceptual bridge between Week 2 bitcell analysis and Week 3 peripheral circuit design.

## Preview of Chapter 11 – Precharge & Equalization Circuit (Week 3)

The next chapter begins **Week 3** by moving outside the bitcell. We will study the **precharge and equalization circuit**, one of the first peripheral blocks activated before every read operation. Topics include:

* Why both bitlines are precharged.
...
* Common design mistakes and debugging.

This marks the transition from **bitcell design** to **SRAM peripheral circuit engineering**, closely following the workflow used in industrial memory design teams.
